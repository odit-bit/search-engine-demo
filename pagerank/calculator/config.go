...
func (c *Config) validate() error {
	...
	if c.DampingFactor < 0 || c.DampingFactor > 1.0 {
		err = multierr.Append(err, errors.New("DampingFactor must be in the range (0, 1]"))
	} else if c.DampingFactor == 0 {
		c.DampingFactor = 0.85
	}

	if c.MinSADForConvergence < 0 || c.MinSADForConvergence >= 1.0 {
		err = multierr.Append(err, errors.New("MinSADForConvergence must be in the range (0, 1)"))
