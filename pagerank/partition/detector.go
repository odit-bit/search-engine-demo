...
var (
	...
	ErrNoPartitionDataAvailableYet = fmt.Errorf("no partition data available yet")
)
...
func (det FromSRVRecords) PartitionInfo() (int, int, error) {
	...
	if err != nil {
		return -1, -1, fmt.Errorf("partition detector: unable to extract partition number from host name suffix")
	}
